...
class _TutorialesScreenState extends State<TutorialesScreen> {
  ...
  void _mostrarDialogoMotivacional(
      BuildContext context, CordaFIU cordaRequerida, String tituloTutorial, bool isDark) {
    final bottomSheetBg = isDark ? GingaColors.surfaceDark : Colors.white;
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        // Build cord ribbon decoration
        BoxDecoration cordaGradientDecoration;
        if (cordaRequerida.colores.length > 1) {
          cordaGradientDecoration = BoxDecoration(
            gradient: LinearGradient(
              colors: cordaRequerida.colores,
              begin: Alignment.centerLeft,
              end: Alignment.centerRight,
            ),
            borderRadius: BorderRadius.circular(6),
          );
        } else if (cordaRequerida.colores.length == 1) {
          cordaGradientDecoration = BoxDecoration(
            color: cordaRequerida.colores.first,
            borderRadius: BorderRadius.circular(6),
          );
        } else {
          cordaGradientDecoration = BoxDecoration(
            ...
            children: [
              ...
              RichText(
                ...
                text: TextSpan(
                  ...
                  children: [
                    const TextSpan(text: 'La lección '),
                    TextSpan(
                      text: '"$tituloTutorial"',
                      style: const TextStyle(fontWeight: FontWeight.bold),
                    ),
                    const TextSpan(
                      text: ' requiere tener la graduación de ',
                    ),
                    TextSpan(
                      text: 'Cuerda ${cordaRequerida.nombre}',
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        color: isDark ? GingaColors.accentAmber : Colors.orange,
                      ),
                    ),
                    const TextSpan(text: ' o superior.'),
                  ],
                ),
              ),
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      height: 10,
                      width: 200,
                      decoration: cordaGradientDecoration,
                    ),
                    const SizedBox(height: 12),
                    if (cordaRequerida.simbolismo.isNotEmpty) ...[
                      Text(
                        cordaRequerida.simbolismo,
                        style: GoogleFonts.montserrat(
                          fontSize: 11,
                          fontWeight: FontWeight.w700,
                          color: isDark ? GingaColors.brandGreen : const Color(0xFF0F9D58),
                        ),
                      ),
                        ...
                        builder: (context, snapshot) {
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final tutorialCordaName = data['corda'] ?? 'Crua';

                              // Comparación de bloqueo
                              final tutorialCorda = CuerdasFIU.encontrarCordaFIU(tutorialCordaName) ?? CuerdasFIU.lista.first;
                              ...
                              return _StaggeredListItem(
                                ...
                                child: Material(
                                  ...
                                  child: InkWell(
                                    onTap: () {
                                      if (isLocked) {
                                        _mostrarDialogoMotivacional(context, tutorialCorda, titulo, isDark);
                                      } else {
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                      ...
                                      child: Opacity(
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            // Botón play circular lateral / Lock Icon
                                            Container(
                                              width: 36,
                                              height: 36,
                                              decoration: BoxDecoration(
                                                color: isLocked ? Colors.grey.withOpacity(0.12) : GingaColors.brandGreen,
                                                shape: BoxShape.circle,
                                              ),
                                              child: Icon(
                                                isLocked ? Icons.lock_outline_rounded : Icons.play_arrow,
                                                color: isLocked ? subtitleColor.withOpacity(0.6) : Colors.white,
                                                size: 20,
                                              ),
                                            ),
                                          ],
